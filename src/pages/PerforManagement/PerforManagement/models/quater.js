...
import {
  ...
  addfinace,
  deleteATquater,
  submitProjectAddFormquater,
  queryattchmentListquater
} from '@/services/api';
...
export default {
  ...
  effects: {
    ...
    *attachmentdel({ payload, callback }, { call, put }) {
      ...
      const response = yield call(deleteATquater, id);
      yield put({
        type: 'queryattchment',
        payload: {
          id:id,
    ...
    *submitProjectAddForm({ payload,callback }, { call, put }) {
      const response = yield call(submitProjectAddFormquater, payload);
      if (callback) callback();
      // if (response) {
    ...
    *queryattchment({ payload,callback }, { call, put }) {
      const response = yield call(queryattchmentListquater, payload);
      const obj = {
