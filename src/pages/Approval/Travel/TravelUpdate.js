...
@Form.create()
class TravelUpdate extends PureComponent {
  state = {
    deptId:null,//负责部门
    deptName:[],
    departmentTreeValue:[],
  ...
  componentWillReceiveProps(nextProps){
    if(nextProps.data.record.id !== this.props.data.record.id){
      ...
      const deptName = record.deptName; //部门名
      const psnId = record.psnId;
      const psnName = record.psnName; //报销人名
      const projectId = record.projectId;
      const projectName = record.projectName; //项目名
      this.setState({
        deptId,
        deptName,
              ...
              <Col xl={{ span: 8, offset: 2 }} lg={{ span: 10 }} md={{ span: 24 }} sm={24}>
                <Form.Item label='出差天数'>
                  {getFieldDecorator('travaldays', {
                    initialValue:record.travaldays?record.travaldays:null
              ...
              <Col lg={6} md={12} sm={24}>
                <Form.Item label="单据状态">
                  {getFieldDecorator('status',{
                    ...
                    <Input placeholder={"单据状态"} disabled/>
                  )}
                </Form.Item>
              </Col>
              <Col xl={{ span: 6, offset: 2 }} lg={{ span: 8 }} md={{ span: 12 }} sm={24}>
