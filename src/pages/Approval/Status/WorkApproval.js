...
@Form.create()
class WorkApproval extends PureComponent {
  state = {
    expandForm: false,
    Application:true,
    color1:true,
    ...
    },
    {
      ...
      render: (text, record) => (
        <Fragment>
          ...
          <a onClick={(e) => this.handleWork(e,record)}>{formatMessage({ id: 'valid.look' })}</a>
        </Fragment>
      ),
    },
  ];
  ...
  handleWork = (e,record)=>{
    e.preventDefault()
    router.push('/approval/statustable/workapplication',{
      query:record
    });
  };
  //展开-收起
