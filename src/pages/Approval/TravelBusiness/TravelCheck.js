...
@Form.create()
class TravelCheck extends PureComponent {
  state = {
    initDate:{},
    opinion:'',
    ss:[],
  ...
  componentDidMount(){
    ...
    this.setState({processInstanceId:record.processInstanceId})
  ...
  onRecord = (record)=>{
    ...
    console.log('详情processInstanceId',record.processInstanceId)
    dispatch({
      type:'TBL/find',
      payload:{
        reqData:{
  ...
  addinfo = (type)=>{
    ...
    const { dispatch } = this.props;
    const { person,processInstanceId,proid,checkid } = this.state;
    let c = {
      reqData:{
        checkman:checkid?Number(checkid):null,
        senderman:user.id,
  ...
  render() {
          ...
          dispatch({
            ...
            callback:(res)=>{
              ...
              if(res.errMsg === "提交成功"){
                ...
                //发消息
                const { dispatch } = this.props;
                const { person,proid } = this.state;
                if(arr[0].type === 'USER'){
                  let c = {
                    reqData:{
                      checkman:arr[0]?Number(arr[0].id):null,
                      senderman:user.id,
                      content:this.state.initDate.billcode?this.state.initDate.billcode:'',
                      state:0,
                      type:9,
    ...
    return (
      ...
      >
        ...
        {
          this.state.initDate.status === '审批进行中'? <Card style={{marginTop:24}}>
